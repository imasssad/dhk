...
class Extendurantee(models.Model):
    ...
    def check_date_fields(self):
        dat = self.env['guarantee.extension'].search([('id', '!=', self.id),('letter_guarantee_id', '=', self.letter_guarantee_id.id) ])
        if dat:
            for line in dat:
                if self.extend_end_date <= line.extend_end_date:
                    raise ValidationError(_('Extend End Date Must Be Greater Than Previous Extension Record Extend End Date.'))
                else:
                    pass
        ...
        if self.extend_end_date and self.end_date:
            ...
            if self.extend_end_date <= self.end_date and self.extend_end_date <= self.extend_start_date:
                raise ValidationError(_('Extend End Date Must Be Greater Than End Date'))
