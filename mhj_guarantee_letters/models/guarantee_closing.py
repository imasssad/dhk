...
class Finishurantee(models.Model):
    ...
    extend_start_date = fields.Date(string='Extend Start Date', compute='compute_date')
    ...
    def compute_date(self):
        for rec in self:
            ...
            if dat.extend_end_date:
                rec.extend_end_date = dat.extend_end_date
            else:
                rec.extend_end_date = None
            if dat.extend_start_date:
                rec.extend_start_date = dat.extend_start_date
            else:
                rec.extend_start_date = None
    ...
    def confirm_button(self):
        for rec in self:
            ...
            close_guarantee_name = 'Close Guarantee Letter of ' + rec.letter_guarantee_id.letter_number
            move = self.env['account.move'].create({
                'journal_id': rec.journal_id.id,
                'date': rec.transaction_date,
                'ref': rec.letter_guarantee_id.letter_number
            })
            self.env['account.move.line'].with_context(check_move_validity=False).create(
                {
                    'move_id': move.id,
                    'account_id': config.bank_expense_credit_account_id.id,
                    'name': close_guarantee_name,
                    'debit': rec.cov_letter_amount,
                })
            remaining_amount = rec.net_letter_amount - rec.cov_letter_amount
            self.env['account.move.line'].with_context(check_move_validity=False).create(
                {
                    'move_id': move.id,
                    'account_id': config.bank_lg_account_id.id,
                    'name': close_guarantee_name,
