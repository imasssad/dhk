...
class RaiseGurantee(models.Model):
    ...
    def _onchange_cover_amount(self):
        for rec in self:
            if rec.letter_amount and rec.raise_amount:
    ...
    def _onchange_cover_amount_percentage(self):
        for rec in self:
            if rec.letter_amount and rec.cover_amount_percentage:
    ...
    def confirm_button(self):
        for rec in self:
            ...
            increase_guarantee_name = 'Increase Guarantee Letter of ' + rec.letter_guarantee_id.letter_number
            ...
            self.env['account.move.line'].with_context(check_move_validity=False).create(
                {
                    ...
                    'name': increase_guarantee_name,
                    'debit': rec.raise_amount,
            ...
            remaining_amount = rec.raise_amount - rec.cover_amount
            self.env['account.move.line'].with_context(check_move_validity=False).create(
                {
                    'move_id': move.id,
            ...
            if rec.is_expenses :
                config = self.env['guarantee.letter.setting'].search([('id', '=', rec.config_id.id)])
                expense_name = 'Increase Gurantee Expense of ' + rec.letter_guarantee_id.letter_number
                move = self.env['account.move'].create({
                    'journal_id': rec.journal_id.id,
                    'date': rec.start_date,
                    'ref': rec.letter_guarantee_id.letter_number
