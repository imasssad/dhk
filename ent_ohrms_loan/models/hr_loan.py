...
class HrLoan(models.Model):
    ...
    def pass_loan_amount(self, employee, loan_amount, loan_id=False):
        if loan_id:
            loan_records = self.env['hr.loan'].sudo().search([('id', '!=', loan_id.id), ('employee_id', '=', employee.id), ('state', 'in', ['draft', 'waiting_approval_1', 'running'])])
        else:
            loan_records = self.env['hr.loan'].sudo().search([('employee_id', '=', employee.id), ('state', 'in', ['draft', 'waiting_approval_1', 'running'])])
    ...
    def _compute_all_loans_amount(self):
        for loan in self:
            all_loans = self.search([('employee_id', '=', loan.employee_id.id), ('state', 'in', ['running', 'complete'])])
            all_loans_amount = sum(all_loans.mapped('total_amount'))
            all_loans_paid_amount = sum(all_loans.mapped('total_paid_amount'))
            loan.all_loans_amount = all_loans_amount
            loan.all_loans_paid_amount = all_loans_paid_amount
    ...
    all_loans_amount = fields.Float(string="All Loans Amount", compute='_compute_all_loans_amount')
    all_loans_paid_amount = fields.Float(string="All Loans Paid Amount", compute='_compute_all_loans_amount')
    ...
    def create(self, values):
        if not self.env.user.has_group('hr.group_hr_user') and not self.env.context.get('portal'):
            employee = self.env['hr.employee'].browse([values['employee_id']])
            loan_pass, approved_amount = self.pass_loan_amount(employee, values['loan_amount'])
            if not loan_pass:
                raise ValidationError(_("Sorry, you cannot create a loan more than approved amount"))
        values['name'] = self.env['ir.sequence'].get('hr.loan.seq') or ' '
        res = super(HrLoan, self).create(values)
        return res

    def write(self, vals):
        if vals.get('loan_amount') and not self.env.user.has_group('hr.group_hr_user') and not self.env.context.get('portal'):
            loan_pass, approved_amount = self.pass_loan_amount(self.employee_id, vals['loan_amount'], self)
            if not loan_pass:
                raise ValidationError(_("Sorry, you already have a loan of approved amount you can not increase that"))
